...
export const HeroPage = () => {
  ...
  // const slug = idSlug ? idSlug : "clark-kent";

  const { data: superheroData, error } = useQuery({
    queryKey: ["hero-data", idSlug],
    queryFn: () => getHeroAction(idSlug),
    staleTime: 1000 * 60 * 5, // 5 minutes
    retry: false,
  });

  //si hay error redirecciona a la pagina principal
  if (error) {
    return <Navigate to="/" />;
  }
  ...
  if (!superheroData) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-gray-500">Cargando datos del superhéroe...</div>
      </div>
    );
  }

  //total de poder
  const totalPower = (data: Hero) => {
    return data.strength + data.intelligence + data.speed + data.durability;
  };

  const averagePower = Math.round((totalPower(superheroData!) / 4) * 10);
